    ...
    if System.is_logged(request):
          if System.is_admin(request):
               if request.method == 'POST':
                    return render (request,'alaapp/projects/modify_project.html',{'nav':'block','modify_project':System.get_navbar_color, 'project':Project.objects.get(id__exact=request.POST.get('id')),'areas':ProjectArea.objects.all(),'time_restrictions':Project.objects.get(id__exact=request.POST.get('id')).time_restriction.all()})   
               else:
                    messages.success(request,'¡RT Creado con éxito!')
                    return render (request,'alaapp/projects/modify_project.html',{'nav':'block','modify_project':System.get_navbar_color, 'project':Project.objects.get(id__exact=request.session['project_id']),'areas':ProjectArea.objects.all(),'time_restrictions':Project.objects.get(id__exact=request.session['project_id']).time_restriction.all()})      
          return redirect('home')
    return redirect('index') 
...
def modify_project_root(request):
    if System.is_logged(request):
          if System.is_root(request):
               if request.method == 'POST':
                    return render (request,'alaapp/projects/modify_project_root.html',{ 'project':Project.objects.get(id__exact=request.POST.get('project_id')),'admins':User.objects.filter(role_id__exact=Role.objects.get(name='ADMIN'))})  
               else:
                    return render (request,'alaapp/projects/modify_project_root.html',{ 'project':Project.objects.get(id__exact=request.session['project_id']),'admins':User.objects.filter(role_id__exact=Role.objects.get(name='ADMIN'))})  
          return redirect('home')
    return redirect('index') 
