...
def index(request):
    ...
    return render(request, 'alaapp/index.html')

def register(request):
    if System.is_logged(request):
        return redirect('home')   
    return render(request, 'alaapp/register.html')
...
def active_account(request):
    try:
        ...
        return render (request,'alaapp/verificate/active_account.html')   
    except KeyError:
        return redirect('index')  
...
def edit_profile(request):
    if System.is_logged(request):
        if System.is_player(request):    
            return render(request, 'alaapp/user/edit_profile.html',{'edit_profile':System.get_navbar_color }) 
        return redirect('home')  
    return redirect('index')      
