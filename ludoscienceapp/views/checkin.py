...
def checkin(request):
    if System.is_logged(request):
        if System.is_player(request):
            return render (request,'ludoscienceapp/checkin/create_checkin.html',{'nav':'block','create_checkin':System.get_navbar_color,'projects':User.objects.get(id__exact=request.session['id']).projects.all()})      
        redirect('home')
